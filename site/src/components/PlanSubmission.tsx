...
export default function PlanSubmission() {
  ...
  function handleSubmit(e: Event) {
    e.preventDefault();
    if (!planUrl.trim()) return;

    let hostname = '';
    try { hostname = new URL(planUrl.trim()).hostname; } catch { hostname = 'unknown'; }

    const speedLabel = downloadSpeed
      ? `${downloadSpeed}${uploadSpeed ? '/' + uploadSpeed : ''}`
      : '';
    const titleProvider = providerName.trim() || hostname;
    const title = `[Plan Submission] ${titleProvider}${speedLabel ? ' — ' + network.toUpperCase() + ' ' + speedLabel : ''}`;
    ...
    const bodyParts = [
      ...
      `| Field | Value |`,
      `|-------|-------|`,
      `| **Provider** | ${providerName.trim() || '_Not provided_'} |`,
      `| **Network** | ${networkLabel} |`,
      `| **Download Speed** | ${downloadSpeed ? downloadSpeed + ' Mbps' : '_Not provided_'} |`,
      `| **Upload Speed** | ${uploadSpeed ? uploadSpeed + ' Mbps' : '_Not provided_'} |`,
      `| **Plan URL** | ${planUrl.trim()} |`,
      `| **CIS URL** | ${cisUrl.trim() || '_Not provided_'} |`,
    ];
    ...
    const params = new URLSearchParams({ title, body, labels: 'plan-submission' });

    window.open(
      `https://github.com/srizzling/ismynbncooked/issues/new?${params.toString()}`,
      '_blank'
    );
    setSubmitted(true);
  }
    ...
    return (
      <div class="bg-surface-raised border border-surface-border rounded-2xl p-6 sm:p-8 text-center">
        ...
        <p class="text-neutral-400 mb-6">
          ...
          <a
            href="https://github.com/srizzling/ismynbncooked/issues/new"
            target="_blank"
            rel="noopener"
            class="text-accent underline hover:text-white"
          >
            click here to create it manually
          </a>.
        </p>
        <button
          onClick={() => {
